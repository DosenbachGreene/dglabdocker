...
RUN apt-get update && apt-get install -y sudo wget dirmngr
...
### Install 4dfp tools ###
WORKDIR /opt/4dfp
RUN curl -O ftp://imaging.wustl.edu/pub/raichlab/4dfp_tools/4dfp_release.txt && mkdir NILSRC && mkdir RELEASE
...
WORKDIR ${NILSRC}
RUN curl -O ftp://imaging.wustl.edu/pub/raichlab/4dfp_tools/nil-tools.tar && tar xvf nil-tools.tar && rm nil-tools.tar
...
WORKDIR ${RELEASE}
RUN curl -O ftp://imaging.wustl.edu/pub/raichlab/4dfp_tools/4dfp_scripts.tar && tar xvf 4dfp_scripts.tar && rm 4dfp_scripts.tar

# install git
RUN apt-get update; apt-get install -y git

# install connectome-workbench
RUN apt-get install -y --allow-unauthenticated connectome-workbench
...
# Fix compiler error in FSL 5.0.10 and disable MIST compilation
...
RUN . ${FSLDIR}/etc/fslconf/fsl.sh && \
	...
	cd ${FSLDIR} && ./build
RUN rm -r ${FSLDIR}/LICENCE ${FSLDIR}/README ${FSLDIR}/build ${FSLDIR}/build.log ${FSLDIR}/config ${FSLDIR}/extras ${FSLDIR}/include ${FSLDIR}/lib ${FSLDIR}/refdoc ${FSLDIR}/src
...
# install fsleyes
RUN apt-get install -y --allow-unauthenticated fsleyes && ln -s $(which FSLeyes) /usr/bin/fsleyes

# clean-up
RUN apt-get remove -y make gfortran && apt-get autoremove -y
WORKDIR /
